...
public:
    ...
    virtual bool IsFile() { return !(flags & FILE_ATTRIBUTE_DIRECTORY); }
    virtual unsigned long long GetFileSize() { return datasize; }
    virtual bool GetType() { return NTFS_HPFS; }
};
...
private:
    ...
    int sector_size;
    QHash<unsigned long, QVector<QNtfsFileRecord>> filesystem;// 构造文件系统树
...
public:
    ...
    virtual QVector<QRecordFile*> GetRootFiles()
    {
        ...
        for (const QNtfsFileRecord& ff : filesystem[MFT_ROOT_INDX])
        {
            result.push_back(&ff);
        }
        return result;
    ...
    {
        QVector<QRecordFile*> result;
        if (filerecord->GetType() != NTFS_HPFS)
        {
            return result;
        }
        unsigned long mftindx = ((QNtfsFileRecord*)filerecord)->mftindx;
        for (const QNtfsFileRecord& ff : filesystem[mftindx])
        {
            result.push_back(&ff);
        }
        return result;
